...
- **Meta command `@group`**: Organize a menu into labeled columns
  ...
  - Optional header alignment via `+left` (default), `+center`, or
    `+right`.
  ...
  - New header colors: `:fg-header` / `--fg-header` (default
    `#7FB4CA`) and `:bg-header` / `--bg-header` (unset by default).

### Changed

- Generated `key_chords.h` now defines `PROPERTY_INT` and `headerBg`.
  Headers generated by older `wk` versions must be regenerated with
  `make from-wks` or `wk --transpile`; see [MIGRATION.md](MIGRATION.md).
